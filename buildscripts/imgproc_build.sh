...
echo "=== Building imgproc tests (Rust only) ==="
...
OPENSSL_INCLUDE_DIR="$STAGING_DIR/usr/include" \
    cargo build -p imgproc --release --target "$RUST_TARGET" -Z build-std=std,panic_abort

OUTPUT="$WORKSPACE/output/imgproc"
install -D -m 755 "target/$RUST_TARGET/release/imgproc" "$OUTPUT"

echo "=== Done: $OUTPUT ==="
echo "Deploy: scp output/imgproc root@atomet.local:/media/mmc/"
